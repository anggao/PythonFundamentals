...
class CommandHandler:
    """
    Simple command lhandler similar to cmd.Cmd from the standard
    library.
    """
    ...
    def handle(self, session, line):
        'Handle a received aline form a given session'
        ...
        try:
            ...
            # If it isn't, response to the unknown command:
            self.unknown(session, cmd)


class Room(CommandHandler):
    """
    A generic environment that may contian one or more users (sessions).
    ...
    def broadcast(self, line):
        'Sned a line ot all sessions in the room'
        for session in self.sessions:
            session.push(line)

    def do_logout(self, session, line):
        'Response to the logout command'
        raise EndSession 
...
class LogoutRoom(Room):
    ...
    the user's name form the server
    """
