...
def randomString(length):
    """
    Returns a random string of letters with the given lenght.
    """
    chars = []
...
class Client(wx.App):
    ...
    def __init__(self, url, dirname, urlfile):
        ...
        Thread is a daemon so it will quit when the application quites),
    ...
    def OnInit(self):
        ...
        """
        win = wx.Frame(None, title="File Sharing Client", size=(400,50))
        bkg = wx.Panel(win)
    ...
    def fetchHandler(self, event):
        """
        Called when the user clicks the 'Fetch' button. Reads the query from
        the text field, and calls the fetch method of the server Node.
